...
| `test-borealis.R` | The Borealis Dataverse source (`R/borealis.R`): DOI normalisation (bare, `doi:`, `https://doi.org/`, dataset URL) and rejection, `.borealis_doi_arg()` accepting a catalogue row, the `X-Dataverse-key` header sent only when `BOREALIS_DATAVERSE_KEY` is set, `.borealis_select_files()` on a synthetic ODESI listing (CSV chosen as data, `.sps` as metadata, SAS/Stata/tab/FWF/missRecode copies skipped, PDFs/codebooks as docs, FWF + `.sav` fallback when no CSV, oversized or restricted docs skipped), the manifest-derived `file_mask` (regex-escaped), StatCan/EFT-first Census resolution (empty cache → Borealis keys; deposited 1971 bundle → EFT keys; `"borealis"`/`"eft"` keywords force a source), `.pumf_borealis_alternative()`, the 14 Borealis Census entries (unique DOIs, listed by `.borealis_registry_collection()`), `pumf_registry_entry(borealis =)` validation, `get_pumf(borealis =)` override merging (same DOI keeps the entry's fixups, another DOI drops them) and argument errors. Network tests (`skip_if_offline("borealisdata.ca")`): `list_borealis_pumf_files()` on a real dataset and an uncached `list_borealis_pumf_catalogue()` |
| `test-statcan-adapter.R` | StatCan catalogue → collection adapter (`.statcan_catalogue_to_collection()`): version keys per series, format preference, empty input; and the Census list falling back to the catalogue's `98m0001x/2023001/` rows when the moved StatCan index page 404s (mocked `rvest::read_html`), with the hard-coded fallback URLs pointing at the same location |
...
| Census (Borealis) | `test-pipeline-census.R` | 1986 (individuals/households/families), 1981 (individuals/households), 1976 (individuals/households/families), 1971 (individuals/households/families × provincial/CMA) | ✓ | ✓ | — | Downloaded from Borealis (ODESI CSV + `.sps`); English-only labels, so bilingual parity is skipped. Resolved automatically when no EFT bundle is deposited. 1971 (individuals, CMA) and 1971 (families, provincial) warn "absent from command files" for codes_supplement injections (TYPE66/TYPE71 0, CMACODE 0) |
