...
# Skipped when offline or on CRAN.

cpss_cache <- withr::local_tempdir(.local_envir = parent.env(environment()))
...
test_that("CPSS v1: pumf_parse_metadata produces canonical CSV files", {
  skip_if_offline()
  skip_on_cran()
  skip_if_not(canpumf:::.version_is_extracted(cpss_vdir()), "Stage 1 not run")
...
test_that("CPSS v1 eng table: PROV column is ENUM in DuckDB", {
  ...
  # PROV or similar province column should be ENUM
...
test_that("CPSS v1: eng and fra tables coexist in same DuckDB", {
  ...
  # Build both (no-ops if already built)
  canpumf:::pumf_build_duckdb(cpss_vdir(), "CPSS", "1", lang = "eng")
...
test_that("CPSS v1 fra vs eng: PROV label differs in French", {
  skip_if_offline()
  skip_on_cran()
  skip_if_not(file.exists(file.path(cpss_vdir(), "CPSS_1.duckdb")),
              "Stage 3 not run")

  r_eng <- canpumf:::pumf_build_duckdb(cpss_vdir(), "CPSS", "1", lang = "eng")
  r_fra <- canpumf:::pumf_build_duckdb(cpss_vdir(), "CPSS", "1", lang = "fra")

  eng <- dplyr::collect(canpumf:::pumf_open_duckdb(r_eng$db_path, "eng"))
  fra <- dplyr::collect(canpumf:::pumf_open_duckdb(r_fra$db_path, "fra"))

  # Column names are the same (variable codes, not labels)
  expect_equal(sort(names(eng)), sort(names(fra)))

  # At least one PROV-like categorical column should differ between languages
  cat_cols <- names(eng)[sapply(eng, is.character)]
  any_differ <- any(sapply(cat_cols, function(col) {
    e <- sort(unique(stats::na.omit(eng[[col]])))
    f <- sort(unique(stats::na.omit(fra[[col]])))
    !identical(e, f)
  }))
  expect_true(any_differ,
    label = "Expected at least one column with different eng/fra labels")
})
