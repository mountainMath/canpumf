...
flowchart TD
    ...
    subgraph S1[\"Stage 1 — locate / download\"]
      direction TB
      CHK{version dir\\nexists?}
      CHK -->|yes, not refresh| EXTR{zip already\\nextracted?}
    ...
    subgraph S2[\"Stage 2 — parse metadata\"]
      ...
      DF --> P5{\"SPSS mono\\n(.sps / *SPSS.txt / .xmf)?\"}
      DF --> P6{\"SPSS .sav?\"}
      P1 & P2 & P3 & P4 & P5 & P6 --> MRG[\"merge_metadata()\"]
      MRG --> WR[\"write variables.csv\\ncodes.csv  layout.csv\"]
    end
    ...
    subgraph S3[\"Stage 3 — build DuckDB\"]
      ...
      RFW & RCS --> FX[\"apply data fixups\\n(str_pad, rename)\"]
      FX --> BSW{BSW mask\\nin registry?}
      BSW -->|yes| RBW[\"join bootstrap weights\"]
      BSW -->|no| NC
      RBW --> NC[\"numeric conversion\\n(missing ranges + na_values)\"]
      NC --> CL[\"code labels → factors\"]
      CL --> WD[\"write DuckDB table\"]
      WD --> EN[\"enforce ENUM types\"]
      EN --> OD[\"open read-only connection\"]
    end

    S3 --> REG[\"register provenance\\n(series, version, lang)\"]
...
| 5 | **SPSS monolithic** | `.sps` file, `*SPSS.txt` file, or `.xmf` file whose first 500 lines contain both `VARIABLE LABELS` and `VALUE LABELS` (checked with `useBytes = TRUE` to tolerate CP850/Latin-1 data) |
...
Handles the single-file SPSS format used by Census (2001–2021), SHS, and
others.  The file contains `DATA LIST`, `VARIABLE LABELS`, `VALUE LABELS`, and
sometimes `MISSING VALUES` and `FORMATS` sections.
...
- **Sentinel detection** — variables whose only VALUE LABELS are sentinel
  phrases ("Not applicable", "Valid skip", "Don't know", etc.) are classified
  as `numeric` with a `missing_low/missing_high` range, not as `character`.
  This prevents spurious NA warnings when numeric values fall outside the label
  set.
...
table, not just the values present in the data.  Unmatched raw values become
`NA` with a warning that shows the first five offending values.
...
The LFS is handled by `lfs_get_pumf()` (delegated from `get_pumf()`).  Instead
of one DuckDB per version, all LFS versions share a single
...
| `data_fixups` | list of `str_pad`, `rename`, `na_values` transforms | `list()` |
