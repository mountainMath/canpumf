...
```{r pipeline-diagram, echo=FALSE, fig.width=12, fig.height=6, out.width="100%"}
if (requireNamespace("DiagrammeR", quietly = TRUE)) {
  DiagrammeR::mermaid("
graph TD
    A[get_pumf series version lang] --> LFS{series == LFS?}
    LFS -->|yes| LFSP[lfs_get_pumf]
    LFS -->|no| S1

    subgraph S1 Stage 1 locate download
      CHK{version dir exists?}
      CHK -->|yes, not refresh| EXTR{zip already extracted?}
      CHK -->|no / refresh| COL[look up collection URL]
      COL --> EFT{EFT-only?}
      EFT -->|yes| ERR[stop: deposit zip manually]
      EFT -->|no| DL[download zip]
      DL --> UZ[robust_unzip]
      UZ --> EXTR
      EXTR -->|yes| S2
      EXTR -->|no| UZ
    ...
    subgraph S2 Stage 2 parse metadata
      MC{metadata already exists?}
      MC -->|yes, not refresh| S3
      MC -->|no / refresh| DF[detect_formats]
      DF --> P1{LFS codebook.csv?}
      DF --> P2{CPSS variables.csv?}
      DF --> P3{SAS cards .lay plus .lbe?}
      DF --> P4{SPSS split vare/vale/_i.sps?}
      DF --> P5{SPSS mono .sps / SPSS.txt / .xmf?}
      DF --> P6{SPSS .sav?}
      DF --> P7{PDF Dictionary?}
      P1 & P2 & P3 & P4 & P5 & P6 & P7 --> MRG[merge_metadata]
      MRG --> WR[write variables.csv codes.csv layout.csv]
    end

    S2 --> S3

    subgraph S3 Stage 3 build DuckDB
      TB{table already in DuckDB?}
      TB -->|yes, not refresh| OD
      TB -->|no / refresh| FF[find data file]
      FF --> FWF{layout.csv exists and file not .csv?}
      FWF -->|yes FWF| RFW[read_fwf]
      FWF -->|no CSV| RCS[read_csv]
      RFW & RCS --> JNK[drop trailing junk rows]
      JNK --> FX[apply data fixups str_pad rename cols_swap]
      FX --> BSW{BSW mask in registry?}
      BSW -->|yes| RBW[join bootstrap weights]
      BSW -->|no| NC
      RBW --> NC[numeric conversion missing ranges + na_values]
      NC --> CL[code labels to factors]
      CL --> WD[write DuckDB table]
      WD --> EN[enforce ENUM types]
      EN --> OD[open read-only connection]
    end

    S3 --> REG[register provenance series version lang]
    REG --> TBL[return lazy dplyr tbl]

  ", height = 650, width = 1200) |>
     htmltools::tagList()
} else {
  cat("Install DiagrammeR to render this diagram: install.packages('DiagrammeR')")
}
```
...
- **`force_numeric`** — character vector of column names to treat as numeric
  regardless of how many VALUE LABELS are declared.  Used when a variable is an
  integer index that the SPSS file mis-classifies as categorical (e.g. SUBSAMPL
  in Census 1971).
- **`codes_supplement`** — named list of `data.frame`s injecting code-label
  rows absent from the SPSS command files.  Each data frame has columns `val`,
  `label_en`, `label_fr`.  Setting `label_en = NA` marks a value as
  intentionally missing (produces a silent `NA` factor entry without a
  warning, and without introducing a spurious factor level).  All entries are
  verified in the override ledger (`tests/testthat/override_verification.csv`).
- **`na_values`** — character vector of raw string sentinels that become `NA`
  for all numeric columns (used for undeclared Census income sentinels).
...
| `data_fixups` | list of `str_pad`, `rename`, `cols_swap`, `force_numeric`, `codes_supplement`, `na_values` transforms | `list()` |
