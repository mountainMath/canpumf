...
- **Labelled missing codes in Stage 3**: `.label_missing_codes()` collects every numeric code whose English *or* French label matches `.missing_prefix_pat`. That is `.missing_pat`, but it also accepts a qualifier after a dash, bracket, colon, comma or semicolon ("NOT STATED - PATH UNKNOWN", "NOT APPLICABLE(DOES NOT DRIVE)"). For each variable that is still `numeric` after `force_numeric` and promotion, `pumf_build_duckdb()` merges these codes into the discrete `missing_codes` set. The merge adds to any parsed or derived range and never replaces it. This is what makes labelled non-response NA when no MISSING VALUES is declared, e.g. GSS Cycle 21 `AGE_DIV_MA1` 999.7. The prefix pattern is **not** used for type classification, where a qualified label still counts as a real category. Codes are compared rounded to 8 places, so a code written as 999.7 matches 9997 read with one implied decimal.
- **MISSING VALUES**: `.spss_parse_missing()` tolerates padding inside the parens (`VALUEH  ( 999999 )/`) and negative values. A single value gives `missing_low == missing_high`, and `lo THRU hi` gives a range. Every `NAMES (values)` group on a (joined) MISSING VALUES statement is parsed, so ODESI files declaring several variables per line (`CMACODE (0)/ FAMSIZE (99)/ …`) and the shared form `A B (99)` are all captured. A comma list of contiguous integers (in any order: `998,999`, `8, 7`) becomes a range, and so does a gapped list of reserved codes in one top band (same digit count, all leading `9`, span < 10: `96,97,99`, `9996,9997,9999`, GSS cycles 8/10). Any other set of discrete values records only its first value (conservative). An empty first slot (`(  ,995 THRU 999 )`, GSS cycles 8-10) is skipped. Use the registry's `missing_codes` fixup when that matters.
...
- **DATA LIST decimals**: no annotation → `fmt_type="F"`, `decimals=0`. `(A)`/`(An)` → character. `(n)` → `decimals=n`. `(Fn.d)` → `decimals=d`. Padded parens (`( 4  )`, GSS cycles 8-10, Census 1981) are accepted.
- **Read-side vs display decimals**: `layout.csv` has an optional `decimals` column holding only the decimals *implied on read* (DATA LIST `(n)`, SAS `w.d` informat). Stage 3 divides fixed-width values by `10^decimals` unless the raw value already contains `.`, before applying the missing range. `variables.csv`'s `decimals` also takes display `FORMATS`, which do not change stored values, so Stage 3 never scales by it. CSV data is never scaled. Old caches without the column read as NA (no scaling).
